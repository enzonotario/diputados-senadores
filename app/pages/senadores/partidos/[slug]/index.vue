...
const { peers: cohesionMembersRaw } = usePeriodFilteredPeers({
  getSource: () => partido.value?.cohesionPeers || [],
  deps: () => partido.value?.cohesionPeers,
});
...
useChamberSeo(() => {
  ...
  return {
    title: name,
    description: `Partido ${name}: ${n} ${
      n === 1 ? "senador" : "senadores"
    } en el período. Integrantes, presentismo y votos en el Senado.`,
    og: {
      kind: "group",
      eyebrow: "partido",
      badge: `${n} ${n === 1 ? "senador" : "senadores"}`,
    },
  };
});
</script>
        ...
        >
          <div class="min-w-0 space-y-2">
            ...
            <p class="text-sm text-muted">
              ...
              {{
                displayed.length === 1
                  ? "senador en el período"
                  : "senadores en el período"
              }}
            </p>
            <div
