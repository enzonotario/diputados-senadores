...
const { data, pending } = await useAsyncData("diputados-comisiones", () =>
...
function onRowSelect(_e: Event, row: { original: ComisionRow }) {
  const to = comisionPath(row.original.id, "diputados");
...
useChamberSeo(() => ({
  title: "Comisiones",
  description:
    "Comisiones de la Cámara de Diputados de la Nación Argentina: integrantes, tipología y diputados que las conforman.",
...
<template>
  <div class="page-container space-y-6">
    <div class="space-y-2">
      <h1 class="text-3xl font-bold">Comisiones de Diputados</h1>
      <p class="text-muted max-w-2xl">
        Listado de comisiones permanentes y especiales, con los diputados que
        las integran.
      </p>
    </div>
      ...
      <DataTableCard :show-periodo-badge="false">
        ...
        >
          <template #nombre-cell="{ row }">
            <NuxtLink
              :to="comisionPath((row.original as ComisionRow).id, 'diputados') || '#'"
